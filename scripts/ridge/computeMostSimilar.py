...
def get_options():
    ...
    parser.add_option("-n", "--nbest", dest="nbest") #nbest list
 
    (options, args) = parser.parse_args()
    return options

def get_most_similar(word, f, t, n):
    if word in f:
        return t.similar_by_vector(f[word],topn=n)
    else:
        print "{} not in embedding".format(word)
        return t.similar_by_vector(np.zeros(f.vector_size),topn=n)
...
if __name__ == '__main__':
    ...
    for word in sourcewords:
        rankedList = get_most_similar(word.strip(),w2v,tw2v,nbest)
